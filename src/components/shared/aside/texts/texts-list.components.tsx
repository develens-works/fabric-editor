...
interface AsideAllTextsOptionProps
{
    id: string;
}

const AsideTextsListOptionComponent: React.FC<AsideAllTextsOptionProps> = ({ id }) =>
{
        ...
        <motion.div initial={ { opacity: 0 } } animate={ { opacity: 1 } } className='nav-wrapper h-full overflow-hidden py-6'>
            <div className='mx-auto -mt-2 mb-2 flex w-[calc(100%-2rem)] gap-2'>
                <button
                    ...
                    onClick={ () => dispatch(setAsideOption('TEXTS')) }
                >
                    <FaArrowLeft/>
                </button>
                ...
                <ul className='grid grid-cols-4 gap-[8px] px-4'>
                    {
                        data.find((item) => item.id === id)?.items.map((item, index) =>
                            (
                                <li key={ `${ item }_all_texts_list_${ index }` } className='relative flex max-h-[calc(269px/4)] min-h-[calc(269px/4)] items-center justify-center align-middle'>
...
export default AsideTextsListOptionComponent;
