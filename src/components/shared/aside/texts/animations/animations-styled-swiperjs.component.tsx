...
import AnimationsItemComponent from '@/components/shared/aside/texts/animations/animations-item.component';

interface TextsAnimationsSwiperProps
{
    id: string;
    title: string;
    items: string[];
...
const TextsAnimationsSwiperComponent: React.FC<TextsAnimationsSwiperProps> = ({ id, title, items }) =>
{
    ...
    return (
        <div className='mt-6 flex h-fit w-full flex-col gap-2'>
            ...
            <Swiper
                ...
                modules={ [Navigation] }
                className='relative size-full max-h-[calc(268px/3)] min-h-[calc(268px/3)]'
            >
                {
                    items.map((item, index) =>
                        (
                            <SwiperSlide key={ item + `_texts_animations_swiperjs_${ title.split(' ').join('_').toUpperCase() }_${ index }` }>
                                <div className='relative flex size-full max-h-[calc(268px/3)] min-h-[calc(268px/3)] cursor-pointer justify-center align-middle'>
                                    <AnimationsItemComponent type='list' system/>
                                </div>
                            </SwiperSlide>
                        ))
                }

                <SwiperSlide className='flex size-full max-h-[calc(268px/3)] min-h-[calc(268px/3)] items-center justify-center align-middle'>
                    <button
                        onClick={ navigateToList }
...
export default TextsAnimationsSwiperComponent;
