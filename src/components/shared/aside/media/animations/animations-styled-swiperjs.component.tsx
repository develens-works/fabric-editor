...
interface MediaAnimationsSwiperProps
{
    id: string;
    title: string;
    items: string[];
...
const MediaAnimationsSwiperComponent: React.FC<MediaAnimationsSwiperProps> = ({ id, title, items }) =>
{
            ...
            >
                {
                    items.map((item, index) =>
                        (
                            <SwiperSlide key={ item + `_media_animations_swiperjs_${ title.split(' ').join('_').toUpperCase() }_${ index }` }>
...
export default MediaAnimationsSwiperComponent;
