...
interface NavAnimationsSwiperProps
{
    id: string;
    title: string;
    items: string[];
...
const NavAnimationsSwiperComponent: React.FC<NavAnimationsSwiperProps> = ({ id, title, items }) =>
{
            ...
            >
                {
                    items.map((item, index) =>
                        (
                            <SwiperSlide key={ item + `_nav_animations_swiperjs_${ title.split(' ').join('_').toUpperCase() }_${ index }` }>
...
export default NavAnimationsSwiperComponent;
